...
const userEndpoint = (router) => {
    ...
    router.get('/api/user/details', auth, async (request, response, next) => {
        try {
            const result = await business.getUserManager(request).get(request.user.userId);
            response.status(200).send(result);
        } catch (error) {
            applicationException.errorHandler(error, response);
        }
    ...
    router.put('/api/user/update', auth, async (request, response, next) => {
        try {
            const result = await business.getUserManager(request).update(request.user.userId, request.body);
            response.status(200).send(result);
        } catch (error) {
            applicationException.errorHandler(error, response);
        }
